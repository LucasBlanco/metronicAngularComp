...
export class Grafico implements OnInit {
    ...
    generarTabla() {
        if (this.nivelDeAnidamiento === 0) {
            let nombreColumnas = [''].concat(Object.keys(this.valoresLocales))
            let datos = JSON.parse(JSON.stringify(this.valoresLocales))
            ...
            if (this.mostrarPromediosHorizontal) {
                ...
                let valores = Object.entries(datos).filter(([key, _]) => key != 'Total').map(([_, value]) => value)
                datos['Promedio'] = (valores.reduce((a, b) => a + (b as any), 0) / valores.length).toFixed(2)
            }
            datos[''] = 'Cantidad'
            return {
                nombreColumnas,
        ...
        if (this.nivelDeAnidamiento === 1) {
            let nombreColumnas = ['Tipo'].concat(Object.keys(this.valoresLocales))
            let datos = Object.keys(Object.entries(JSON.parse(JSON.stringify(this.valoresLocales)))[0][1]).map(
                tipo => ({ 'Tipo': tipo })
            )
            ...
            Object.entries(this.valoresLocales).forEach(
                ([key1, value1]) => Object.entries(value1).forEach(
                    ([key2, value2]) => {
                        let i = datos.findIndex(d => d['Tipo'] === key2)
                        datos[i][key1] = value2
                    }
                )
            )
    ...
    obtenerDatosDiagrama() {
        let datos: { datasets: { data: any, label: any, borderColor: string, backgroundColor: string, yAxisID?: any, fill?: any }[], labels: Array<string> }
        ...
        if (this.nivelDeAnidamiento === 1) {
            ...
            let data = Object.keys(Object.entries(this.valoresLocales)[0][1]).reduce(
            ...
            let datasets = Object.entries(data).map(
                ([key, value]) => {
        ...
        if (this.nivelDeAnidamiento === 2) {
            ...
            let hojas = Object.entries(this.valoresLocales).map(
                ([_, value]) => value
            )


            let data = Object.entries(hojas[0]).map(
                ([key, value]) => ({
